...
export async function serialize(sock, m) {
    ...
    m.adReply = (text, title, body, thumbnail, source, large = false, options = {}) => {
        let textStr = String(text)
        let mentions = [...textStr.matchAll(/@(\d+)/g)].map(v => v[1] + '@s.whatsapp.net')
        return sock.sendMessage(m.from, {
            text: textStr,
            mentions: options.mentions || mentions,
            contextInfo: {
                ...
                externalAdReply: {
                    ...
                    mediaType: 1,
                    thumbnailUrl: thumbnail || config.thumbnail1,
